...
class Program
    {
        static void Main(string[] args)
        {
            ...
            var res=con.CreateReader("SELECT top 20 am.code_wares FROM dbo.dw_am am WHERE am.code_warehouse=9 order by code_wares");
            while (res.Read())
            {
                ...
                var res2 = con2.CreateReader("SELECT dbo.GetPrice(9, " + CodeWares.ToString() + ", NULL, 1)");
                res2.Read();
                watch.Stop();
